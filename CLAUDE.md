...
3. **Cluster health sweep — expected result is no error.**
   ...
   - **Per-pool / per-PVC capacity under the READONLY SA — use the apiserver proxy, not the toolbox (found 2026-08-29).** `pods/exec` is denied, and the CephCluster CR status is too coarse for "how full is that pool". Both of these return 200 under `~/.kube/config-readonly`, no exec and no break-glass:
  ```bash
  ...
  ```
  The two agree: on 2026-08-29 `media-data-pvc` usedBytes 3,535,487,893,504 vs `ceph_pool_stored{pool_id="17"}` 3,535,497,592,832 (9.7 MB apart), which also proved that PVC is the SOLE consumer of the `cephfs-bulk-hdd` EC pool.
