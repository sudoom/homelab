...
**Mons advertise FRONTNET addresses while OSDs advertise BACKNET — this is CORRECT, do not "fix" it.** `rook-ceph-mon-endpoints` and `ceph mon dump` both show `192.168.1.7/8/9`, which looks like `addressRanges` failing to apply. It is not. Verified 2026-08-28: `ceph config get mon public_network` = `192.168.10.0/24`, `cluster_network` = `192.168.10.0/24`, and **all six OSDs advertise `192.168.10.2/3/4` on both their public and cluster addrs** (`ceph osd dump`). A client contacts a mon once over the frontnet to fetch the OSDMap (a few KB of control plane), then does all data IO directly against the OSDs on the 10G backnet. **Client data has never traversed the 1G link.** Corollary worth internalising: **the measured `cephfs-hdd` throughput (read 76.6 MB/s / write 22.1 MB/s, 2026-06-12) is NOT a network cap** — it is what EC 2+1 across three spindles actually delivers single-stream, with the write figure reflecting EC write amplification. Do not go looking for a 1G bottleneck to explain it. (Second corollary: both networks are load-bearing for storage — frontnet down = no mon contact, backnet down = no OSD IO, which is why a single node's backnet NIC coming up DOWN after a reboot breaks all its RBD mounts.)
