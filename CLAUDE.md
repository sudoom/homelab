...
- **Active consumers:** Loki (33+ GiB / 53k+ chunks in `ceph-objectstore.rgw.buckets.data` as of 2026-05-10). **OADP 1.5.0 is INSTALLED and wired to RGW** (verified 2026-08-28: DPA reconciled, BSL `default` Available at `rook-ceph-rgw-ceph-objectstore.rook-ceph.svc` bucket `oadp`, prefix `velero`, 3 node-agents + velero Running, `uploaderType: kopia`) — **First backup AND first restore taken 2026-08-28** (the `cephfs-hdd` → `nfs-truenas-keepers` rehearsal, PASSED, verified across four layers — see `tests/oadp-sc-migration-rehearsal/`). The FSB backup+restore path is therefore proven at ~20 MB scale; the daily `Schedule` remains `paused: true`. **A second BSL `truenas-garage` now exists** (garage S3 on the NAS, reached at `192.168.1.25:30188` on the FRONTNET — pods cannot reach the backnet) precisely because the RGW BSL structurally cannot hold the media PVC: **Capacity ceiling on that BSL:** a Velero file-system backup writes to object storage ONLY (no direct PVC→PVC path exists), and the RGW data pool is replicated ×3 on the same 3 HDD OSDs — so backing up the 3.21 TiB `media-data-pvc` would need **9.63 TiB raw against 6.66 TiB available**. It does not fit, and no compression closes that gap on an already-compressed media library. OADP is right-sized here for config/small-PVC backup and for proving the restore path; it is NOT the tool for bulk volume migration on this cluster. (**CNPG backups did NOT land on RGW** — RGW is on-cluster, not a valid *offsite* target. They went OFFSITE to **Cloudflare R2** via the **barman-cloud plugin** 2026-06-27: `components/operators/cnpg-barman-plugin/` + per-cluster `ObjectStore`/`plugins:` for media-postgres + immich-postgres. Gotchas — WAL `compression: gzip` is fine, but **DROP `data.compression` entirely for R2** (2026-07-05): gzip base backups upload unequal-length multipart parts → R2 rejects them with `InvalidPart: all non-trailing parts must have the same length` once the DB exceeds the ~5 MB single-part threshold (immich hit this after its library import grew the DB; EnterpriseDB/barman#954). Uncompressed base parts are uniform → R2 accepts. No compressor fixes it (CRD only accepts gzip/bzip2/snappy, all stream variable-length parts); `--min-chunk-size=5GB` "single part" is a time-bomb at R2's 5 GiB ceiling. WAL is single-part so stays gzip. (CRD also rejects `zstd`.) `serverName` goes in `Cluster.spec.plugins[].parameters`, not the ObjectStore; the boto3≥1.36 checksum env is required; the sidecar is a NATIVE sidecar (initContainer, invisible in `.spec.containers`) with per-command creds, so **verify via the bucket + sidecar logs, NOT CNPG status conditions**. Full saga: `blog/blog-cnpg-draft.md`.) **WAL archiving depends on pod→external egress** — a node outage re-trips the OVN `ovnkube-node` egress break (see network pre-flight) and **kills R2 archiving on BOTH clusters**; a long-enough outage then fills the WAL PVC → CNPG **`Not enough disk space`** safe-mode deadlock (**CNPG will NOT auto-expand from that state** — its reconcile bails on the unreachable `/pg/status` before the resize step, so bumping `Cluster.spec.storage.size` alone no-ops; PVCs stay `requested=<old>`). **Recovery order:** restart all 3 `ovnkube-node` → manually `oc patch` each instance PVC `spec.resources.requests.storage` larger (RBD `allowVolumeExpansion=true`; matches the committed CR size = no drift) → delete the crashlooping pods to remount the grown fs + clear the kubelet backoff → Postgres starts, flushes the WAL backlog. **Detection shipped 2026-07-08**: `CNPGWALArchiveFailing` (warn 15m) + `CNPGWALArchiveFailingCritical` (crit 1h) PrometheusRules on `increase(cnpg_pg_stat_archiver_failed_count[15m])>0`, per namespace in UWM (`components/apps/{cnpg-clusters,immich}/templates/prometheusrule-wal-archive.yaml`) — so an archive stall now fires within 15 min. The residual is *delivery* (no external Alertmanager route → it only reaches the in-console alert list, which browns out with the same-pool stack; README observability TODO), NOT detection. **`media-postgres` PVC grown 10Gi→20Gi permanently 2026-07-02** after exactly this (2-day node4 power outage); full chronology in `blog/blog-cnpg-draft.md` 2026-07-02.
...
- **NO CONSUMERS AS OF 2026-08-30 — `cephfs-hdd` is an empty tier.** `media/media-data-pvc` (RWX, 4 Ti quota, 3.2155 TiB) was its first and only consumer from 2026-06-15 and **migrated to TrueNAS NFS on 2026-08-30** (`tests/media-truenas-migration/`, verified byte-identical with hardlinks intact: 2205 paths → 1465 inodes on both sides). All six media consumers now mount `192.168.10.10:/mnt/tank/media`. **The PVC and its data are DELIBERATELY RETAINED** through a one-week soak — `cephfs-hdd` is `reclaimPolicy: Retain` and this is the only rollback that is not a 3.2 TiB re-copy. Do not reclaim before ~2026-09-06.
  - **Consequence to follow through:** `media-data-pvc` was the SOLE consumer of the `cephfs-bulk-hdd` EC pool (proved 2026-08-28: `ceph_pool_stored{pool_id="17"}` and the PVC's kubelet `usedBytes` agreed to within 9.7 MB). Once reclaimed, that pool is empty and the 3 HDD OSDs serve only the RGW data pool (Loki + OADP). With the queued Loki→garage move done, the whole HDD tier has no purpose: retiring it frees one 3.5" bay per node, ~15–20 W, and makes `CephPGImbalance` disappear, since that false positive exists ONLY because two device classes are averaged together. See README TODO.
  - Historical, still true of the tier itself: a CephFS teardown/recreate needs the manual `ceph fs new … --force`. The EC pool shares the 3 HDD OSDs with the replicated RGW data pool, so CephFS-used (×1.5 raw) + RGW-used (×3 raw) must stay under ~10.9 TiB HDD raw. PVC `storageClassName` is immutable, so a backend flip needs a SECOND PVC and a consumer repoint — **not** `Replace=true` on a data PVC (footgun: would delete+recreate it on any future diff). That two-PVC pattern is what both the keepers and media migrations used.
...
- **CephFS post-node-outage recovery is DIFFERENT from RBD's — a wedged node-level CephFS mount, and the fix is a kubelet restart, NOT the umount/rmdir chase (2026-07-26).** After a node outage churns connectivity, that node's kernel CephFS *stage* mount (`.../plugins/kubernetes.io/csi/rook-ceph.cephfs.csi.ceph.com/<volhandle>/globalmount`) can go stale — the MDS session died but the mount lingers. Every CephFS consumer scheduled to that node (the media stack's `media-data-pvc` RWX `/data`) then hangs `MountVolume.SetUp failed … globalmount: permission denied`; the app is `Running`/`ready` (kubelet's probe doesn't test the mount) but sees `/data` as an ESTALE/dead mount (e.g. radarr/sonarr root folders `Unavailable`). **This survives cephfs-nodeplugin restarts** (userspace restart ≠ kernel unmount) — same "chase the cause not the lock" trap as RBD. **DO NOT hand-chase it** the way I did on 2026-07-26 (host `umount -f/-l` the globalmount → `rmdir` it → `rm -rf` the vol dir): each step just walked the error forward `permission denied → not a mountpoint → file does not exist` and left ceph-csi's staging state corrupted, so *new* pods on that node kept failing. **The clean fix, in order of preference:** (1) get the consumers OFF the bad node — `oc adm cordon <node>` (operator-run; `oc adm cordon` AND `oc patch node …unschedulable` are BOTH classifier-blocked even under break-glass, so hand the cordon/uncordon to the operator) then `oc delete pod` the stuck ones so they reschedule to a healthy node (RWX → they can land anywhere; confirmed they recover instantly elsewhere); (2) **`sudo systemctl restart kubelet` on the bad node** (SSH) — re-reconciles all volumes from scratch, recreating a clean staging dir + metadata; THEN uncordon. **Never reboot the node to fix this** — a reboot re-trips the whole ovnkube-egress + backnet-NIC + stuck-VA cascade (see network pre-flight); the kubelet restart is seconds and disturbs nothing. **Gap:** nothing keeps the 6 CephFS consumers off a poisoned node (no anti-affinity/taint), so on uncordon they can drift back onto it until kubelet is restarted there. Full saga: `blog/blog-ntp-dns-cluster-outage-draft.md` Part 3.
...
**Mons advertise FRONTNET addresses while OSDs advertise BACKNET — this is CORRECT, do not "fix" it.** `rook-ceph-mon-endpoints` and `ceph mon dump` both show `192.168.1.7/8/9`, which looks like `addressRanges` failing to apply. It is not. Verified 2026-08-28: `ceph config get mon public_network` = `192.168.10.0/24`, `cluster_network` = `192.168.10.0/24`, and **all six OSDs advertise `192.168.10.2/3/4` on both their public and cluster addrs** (`ceph osd dump`). A client contacts a mon once over the frontnet to fetch the OSDMap (a few KB of control plane), then does all data IO directly against the OSDs on the 10G backnet. **Client data has never traversed the 1G link.** Corollary worth internalising: **`cephfs-hdd` throughput is NOT a network cap** — it is what EC 2+1 across three spindles delivers, with the write figure reflecting EC write amplification. Do not go looking for a 1G bottleneck to explain it. **Re-measured 2026-08-28 (supersedes the 2026-06-12 Phase 5 numbers): single-stream read 124 MB/s, 3 parallel streams 170 MiB/s aggregate** — vs read 76.6 MB/s / write 22.1 MB/s on 2026-06-12, i.e. the old figure was stale by ~1.6× and had been quoted forward for months. **Quote the date with the number, and re-measure before sizing anything on it.** **All measured throughput figures for all three backends — Synology, CephFS-HDD and TrueNAS — live in `data/storage-throughput.md`, each with the client/stream count and method that produced it. Read that before quoting any number from here, and add new measurements there rather than inline.** The 170 MiB/s aggregate is 1.43 Gbps — above 1 Gbps line rate — which independently proves the read path is not on the frontnet. (Second corollary: both networks are load-bearing for storage — frontnet down = no mon contact, backnet down = no OSD IO, which is why a single node's backnet NIC coming up DOWN after a reboot breaks all its RBD mounts.)

**POD-NETWORK → STORAGE-BACKNET EGRESS DOES NOT WORK (found 2026-08-28).** Everything that reaches `192.168.10.0/24` today does so from the **host stack**: Ceph is `network.provider: host`, and `csi-nfs-node` + `csi-nfs-controller` are both `hostNetwork: true`. A pod on the OVN pod network (podIP `10.128.0.0/14`) **cannot** reach `192.168.10.x`. Confirmed when the Velero `truenas-garage` BSL failed `dial tcp 192.168.10.10:30188: i/o timeout` and a 90-second `ss` watch **on the NAS saw ZERO inbound SYNs** — the packets never arrive, so this is an egress-side drop, not an asymmetric-reply problem (TrueNAS *would* also misroute the reply: `ip route get 10.130.1.157` → `via 192.168.1.1 dev eno1`, the frontnet gateway, which has no route to the pod CIDR — but that never comes into play). **Pod → frontnet LAN works fine** (`synology-cert-sync` reaches `192.168.1.2` nightly), so the workaround for any pod-network client of a LAN service is to reach it on `192.168.1.x`. **Practical rule: if a POD (not the kubelet, not a hostNetwork DaemonSet) needs to talk to the NAS, point it at `192.168.1.25`, not `192.168.10.10`.** The data path is unaffected — NFS is kernel-mounted by hostNetwork CSI pods, so media/immich/keepers traffic still rides the 10G backnet; only pod-originated traffic (Velero's S3 client) takes the 1G frontnet. **The 1G leg is almost certainly NOT the bottleneck**: `cephfs-hdd` was measured on 2026-06-12 (Phase 5 validation, `blog/blog-hdd-tier-rollout-draft.md`) at **read 76.6 MB/s, write 22.1 MB/s** one-client-one-stream — 0.61 Gbps read, comfortably under 1 Gbps. So CephFS read, not the network, is the constraint on any bulk move off this tier. (An earlier note here claimed ~200 MB/s; that was an unmeasured guess, 2.6× too high, corrected 2026-08-28.) Budget a 3.21 TiB move at **~12 h single-stream**, less if the copier parallelises across files.
