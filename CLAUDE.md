...
- **3 NVMe OSDs in service (`osd.0-2`), one per node.** The 3 HDD OSDs (`osd.3-5`, added 2026-06-12) still exist but hold **zero PGs** since the 2026-09-07 decommission and are pending removal from `cephClusterSpec.storage.nodes[].devices` + physical pull. Failure domain is `host` (labels `fd-a/fd-b/fd-c`, one per node), so the no-drain constraint below applies to every OSD.
...
- **`.mgr` is pinned to the NVMe CRUSH rule** by `components/storage/rook-ceph-cluster/templates/mgr-pool-crush-rule.yaml`. Ceph creates `.mgr` itself, so no CR owns it and it defaults to `replicated_rule`, which targets bare `default` — every device class. That made it the last thing holding PGs on the HDD OSDs after every HDD pool was deleted, and it appears in no list of "HDD pools" because it is not one. A fresh bootstrap recreates it on the default rule, hence the Job.
