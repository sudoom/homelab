...
- **3 NVMe OSDs, one per node — that is the whole cluster (`osd.0-2`).** The 3 HDD OSDs (`osd.3-5`, added 2026-06-12) were purged and physically pulled 2026-09-08; root `default` weight is now 1.39737 (3 x 0.46579). Failure domain is `host` (labels `fd-a/fd-b/fd-c`, one per node), so the no-drain constraint below applies to every OSD. **CAPACITY IS NOW THE BINDING CONSTRAINT, and the HDD removal is what made it visible:** `nvme-replicated` sits at **80.35% / MAX AVAIL 89 GiB** (2026-09-08), where Ceph's `nearfull` warning trips at 85%. It read 76.6% an hour earlier the same day. The 11 TiB of empty HDD used to sit in `ceph -s`'s totals and hid this completely — do NOT read cluster-level `avail` as headroom, read the per-pool `MAX AVAIL` from `ceph df`. Reclaim levers, in order: `fstrim` (the `node-fstrim` DaemonSet — freed RBD blocks are not returned without discard, see RBD CSI quirks), then the CNPG `ceph-rbd-snapshot` volume snapshots on 7d retention (`rbd du` showed 246 GiB allocated against 377 GiB pool-stored, and snapshots are the likeliest gap).
