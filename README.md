...
- [ ] **Encryption in transit — OVN-Kubernetes IPsec — ATTEMPT 1 ROLLED BACK 2026-05-20.** Enabling `mode: Full` triggered an MCO master-pool reroll (kernel modules + NM service install via MachineConfig — wasn't flagged pre-apply) which then surfaced multiple cluster-breaking interactions: (a) Loki ingester PDB MinAvailable=2 with 2 replicas blocked drain permanently, (b) image pulls extremely slow across IPsec (~15-19 min for ceph images), (c) RGW + router anti-affinity left only 1 schedulable node when routers re-balanced, (d) post-IPsec-reboot node could not reach etcd on host-network IPs (`192.168.1.7:2379`, `192.168.1.9:2379`) cross-node — root cause TBD. Rolled back via `oc patch network.operator/cluster` directly (ArgoCD repo-server was timing out under cluster stress). Pre-flight checklist for next attempt — full chronology + lessons in `blog/blog-security-hardening-draft.md` 2026-05-20 section. **Do not re-enable until**: (1) Loki ingester drain headroom restored (see TODO below), (2) Kube Descheduler installed (next-priority, see below), (3) cross-node host-network IPsec interaction diagnosed, (4) MTU pre-flight (try `ovnKubernetesConfig.mtu: 1380` to leave room for ESP overhead).
- [ ] **Loki ingester PDB drain-headroom problem** — surfaced 2026-05-20. `logging-loki-ingester` PDB has `MinAvailable: 2`. We dropped ingester replicas 3→2 during the OSD_FULL emergency. With MinAvailable=2 / 2 replicas → ALLOWED DISRUPTIONS=0 → ANY voluntary eviction (MCO drain, node drain for maintenance) is blocked permanently. The LokiStack CRD does NOT expose PDB tunables (only `replicas`/`nodeSelector`/`podAntiAffinity`/`tolerations`). Three paths forward: (a) bump replicas back to 3 (undoes WAL-space savings, ~480 GiB raw cost; user rejected 2026-05-20), (b) patch the PDB to MinAvailable=1 and re-patch every time loki-operator reconciles (operational toil — same dance every drain), (c) upstream feature request to expose PDB knob in LokiStack CRD. Decide before next MCO event.
