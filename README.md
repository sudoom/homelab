...
- [ ] **CephFS storage class** for ReadWriteMany workloads (currently RWO-only) — **Phase 4** of the HDD-tier rollout. **HDD burn-in COMPLETE 2026-06-12: all 10/10 HGST HUS726040ALA610 4TB PASSED** (long self-test clean, 0 reallocated/pending/offline/CRC) + 2 Intel DC SSDs validated+wiped (S3510 480GB boot-spare, S4610 960GB Synology backup); full per-drive record in `data/hdd-burnin-2026-06-10/`. **OSDs LIVE — Phase 1+2 done 2026-06-12** (`dfb706b`): osd.3/4/5 on the 3 HDDs (1 per host) added via by-id to the `rook-ceph-cluster` chart's `cephClusterSpec.storage.nodes[].devices`, auto-classed `device_class=hdd`, **220 pgs active+clean throughout** (additive — zero data movement, zero degraded window). Raw HDD capacity ~10.9 TiB / ~3.6 TiB usable at size=3, **idle until a pool targets it**. P5090 chassis = 1× 3.5" bay each → **3 active OSDs + 7 shelf spares** (stored raw). Key procedure lessons (all in `blog/blog-hdd-tier-rollout-draft.md`): used HGST pulls carry **DDF firmware-RAID** → wipe (`mdadm --stop`+`wipefs`) on *every* insertion ("validated" ≠ "stored raw" — all 3 in-service drives re-assembled DDF on re-seat); SATA-SSD erase uses `blkdiscard` (hdparm absent on SCOS) + an allowlist gate (ROTA=1 can't protect the boot SSD). **Remaining: Phase 4** — CephFS chart: one `CephFilesystem`, metadata pool on NVMe + two data pools (NVMe + HDD); bulk media stays on NFS/Synology (HDD tier sized for Immich + Jellyfin metadata + OADP, not video).
