...
components/
  ...
  storage/
    rook-ceph-cluster/     # CephCluster CR via upstream rook-ceph-cluster subchart + local extras
                           ...
                           # object bucket SC, Prometheus scrape RBAC)
    ceph-object-store/     # CephObjectStore (RGW) + Route + PrometheusRule
    nfs-csi/               # NFS CSI driver for legacy/external mounts
    ceph-cluster/          # DEPRECATED (replaced by rook-ceph-cluster); disabled in root-app
    ceph-storage-classes/  # DEPRECATED (replaced by rook-ceph-cluster); disabled in root-app
  apps/
...
- [ ] **Storage stack re-bootstrap on `provider: host` + `addressRanges` — chart audited, teardown clean, fresh bootstrap queued for next session** — 2026-05-14 went through a multus rebuild (Phases 4-6 shipped multus, measured 1.6 GB/s 1M seqread vs. 89 MB/s 1M seqwrite), then a Phase 6.5 attempt to split the NADs onto disjoint /26s broke kRBD mount in a way that survived chart revert + required full teardown. Rebuild on `provider: host` + `network.addressRanges: 192.168.10.0/24` chosen as the simpler shape — confirmed via baseline measurement that host on the kubelet-registered IP (1G frontnet) caps at ~62 MB/s wr / ~182 MB/s rd (matches replication-amplification math), and `addressRanges` is the documented fix to push Ceph daemons onto the 10G backnet. End of session: cluster fully torn down (3/3 cleanup-jobs Complete, all leftover state swept per the new "Clean teardown procedure" in CLAUDE.md), chart at the target shape (commit `cd081c4` + post-test minor tweaks committed). **First move next session**: flip `rook-ceph-cluster.enabled: true` + `ceph-object-store.enabled: true` in root-app. Expected: bootstrap straight to 10G-backnet binding, OSDs at `.10.{2,3,4}` from first boot, libaio test verifies. After that, the original consumer re-enable sequence (Phase 7+ from yesterday's plan: `monitoring-config`, `logging-stack`, `grafana-config`, `gatus`, `media`, plus the media-data-pvc NFS rebind). Multus parked indefinitely; if it ever comes back, `kubectl-rook-ceph multus validation run` is mandatory pre-flight per CLAUDE.md. Full chronology in `blog/blog-multus-ceph-migration-draft.md`.
