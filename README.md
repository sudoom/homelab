...
- [ ] **Multus migration take 2 — fresh CephCluster rebuild IN FLIGHT, Phases 2+3 shipped 2026-05-13 evening** — The 2026-05-12 `host → "" → multus` rollback established that a live-cluster transition through an empty-provider intermediate is a deadlock trap on 3-OSD-no-drain topology. Take 2 sidesteps the empty-intermediate by **rebuilding** the cluster on `provider: multus` from the first daemon. Tonight: Phase 2 (consumer drain — disabled `media`/`logging-stack`/`grafana-config`/`gatus`/`monitoring-config`/`ceph-object-store` in root-app, all PVCs cleanly cleared via CSI; CMO surprised us by also auto-clearing the Prometheus/Alertmanager PVCs when their ConfigMap disappeared) + Phase 3 (Ceph teardown — `cleanupPolicy.confirmation: "yes-really-destroy-data"` + disable rook-ceph-cluster → Rook ran per-node `cluster-cleanup-job` that zapped /dev/nvme0n1 on each node in 4-5s, cleaned up `/var/lib/rook`). **Cluster end-of-night state: 0 Ceph CRs, all 3 OSD disks zapped + ready, NADs (`ceph-cluster`/`ceph-public`) + ceph-shim NNCPs preserved for Phase 4.** Phase 4 (tomorrow): flip `network.provider host → multus` + add NAD selectors + drop the cleanup confirmation, re-enable rook-ceph-cluster. Phase 5+: bring up ceph-object-store (RGW on pod IP — drop the router-anti-affinity hack), then re-enable consumers. Full chronology + Phase 3 zap logs in `blog/blog-multus-ceph-migration-draft.md`. **Rollback path if Phase 4 also fails**: revert provider to `host`, re-enable; +1h recovery.
