...
- [ ] **Logging stack: Loki Operator + OKD Logging (OKDerator)** — paired adoption. Loki Operator owns the `LokiStack` CR (the log store: distributor / ingester / querier / compactor / query-frontend); OKD Logging owns the `ClusterLogForwarder` CR + Vector DaemonSet (the collection / shipping pipeline). Wire Grafana as a Loki datasource so logs + metrics share one pane. **Storage:** `LokiStack` requires an S3-shaped backend (no filesystem mode); plan is `CephObjectStore` on NVMe interim → flip `dataPool.deviceClass` to `hdd` once the chassis has HDDs (metadata pool stays on NVMe permanently).
