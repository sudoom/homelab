...
- [ ] **`logging-loki-ingester-2` WAL stuck at 99.83%** — surfaced by 2026-05-13 end-of-session sweep. `wal-logging-loki-ingester-2` PVC sits at 99.83% used (~10Gi quota), zero delta over 30 min, ingester-2 flush rate ~0.033/sec (one chunk per 30s vs. hundreds/min steady-state). Likely accumulated during the 21h RGW outage (KubePersistentVolumeFillingUp went warning 07:05Z, critical 12:26Z — both pre-RGW-recovery at 11:59Z); now stuck despite S3 path being healthy. The other two ingesters' WAL are at 1-2%. Suggested action: `oc -n openshift-logging delete pod logging-loki-ingester-2` to force WAL replay + flush (mutation; not done at goodbye). If the WAL doesn't drain post-restart, dig into the loki-operator's ingester reconciliation — there may be a config-level limit being hit. Two `KubePersistentVolumeFillingUp` alerts (critical + warning) will clear automatically once WAL drains.
