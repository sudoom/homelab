...
- [ ] **Encryption in transit — OVN-Kubernetes IPsec** — flip `spec.defaultNetwork.ovnKubernetesConfig.ipsecConfig.mode: Full` on the cluster `Network` CR. Encrypts the geneve overlay between all nodes, so every pod-to-pod packet crossing a host is IPsec-tunneled instead of just integrity-checked. Closest in-transit analog to the 2026-05-18 etcd-at-rest enable; one config knob, no app changes. Cost: ~10-15% CPU overhead on east-west throughput (homelab has headroom). Pairs with the Ceph msgr2 secure-mode item in `Queued — storage` to cover OSD↔OSD traffic that doesn't ride the overlay. Validate with `oc get network.config/cluster -o yaml` + `tcpdump -i any esp` on a node — IPsec packets are ESP-protocol, easy to confirm.
...
- [ ] **CNPG: migrate `barmanObjectStore` to Barman Cloud Plugin** — CNPG 1.30 removes native barman support; the operator already emits a deprecation warning on every apply touching `spec.backup.barmanObjectStore` and `externalClusters[].barmanObjectStore`. Plugin contract is install-once + re-shape the chart's backup block + re-shape `tests/cnpg-restore-drill.yaml`. Not urgent (1.x line is at 1.27 today) but it's a hard removal, not a soft hint. Write-up in `blog/blog-cnpg-draft.md` 2026-05-20 section.
...
- [ ] **Kube Descheduler (OKDerator) — evaluate after next MCO reboot** — re-evaluates pod placement at runtime; useful for unsticking anti-affinity-preferring workloads that clustered onto 2 nodes during a node reboot and stayed there afterward. **Gate on observing the symptom first** (`oc get pods -A -o wide` after the next MCO update — if ArgoCD components / Grafana / Vector / servarrs visibly clustered on 1-2 nodes, install). Use the `AffinityAndTaints` profile; **skip `LowNodeUtilization` + `PodLifeTime`** (the former is meaningless on a 3-node cluster, the latter is chaos-style without a reason). Exclusion list is load-bearing — must exempt `priorityClassName: system-cluster-critical`, namespaces `rook-ceph` / `openshift-gitops` / `openshift-monitoring` / `openshift-logging`, and CNPG-labeled pods. Without exclusions, descheduler will happily evict the CNPG primary or a Rook OSD.
