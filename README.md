...
- [ ] **Migrate DNS from pi-hole (`192.168.1.12`) to `technitium` (running Technitium DNS Server)** — *priority bumped 2026-05-12 after the cluster overwhelmed pi-hole with a search-suffix leak storm: ~700 k queries in 18 h, sustained ~11 qps, on-host CoreDNS logs full of `read udp 192.168.1.X->192.168.1.12:53: i/o timeout`.* Root cause is structural and cluster-wide: pods inherit `search ... okd.sudops.pl` from the host resolv.conf via `dnsPolicy: ClusterFirst`, so with `ndots:5` every `<svc>.<ns>.svc.cluster.local` lookup search-expands to `<...>.svc.cluster.local.okd.sudops.pl`. That falls through cluster CoreDNS (`forward . /etc/resolv.conf`) → on-host baremetal CoreDNS → its `DNSUpstreams` → pi-hole → pi-hole's `okd.sudops.pl` conditional forwarder → timeout → caller retries. Amplifier: `dns.operator.openshift.io/default` ships with `cache: { negativeTTL: 0s, positiveTTL: 0s }`, so nothing is cached and every miss hits the upstream chain again. Heavy emitters are Loki memberlist (`gossip-ring`, `query-frontend-grpc`) and ArgoCD `repo-server-grpc`, but the leak applies to every cluster service lookup. **Decision (2026-05-12):** live with the leak; pi-hole is going away anyway, don't fix on a box that won't exist next month. **Planning doc**: `blog/blog-technitium-dns-migration-draft.md` — shape decided (RPi 3B+ on same IP `192.168.1.12`, Technitium DNS Server with authoritative split-horizon for `okd.sudops.pl`, secondary on RPi Zero 2W as future addition, recursive + DNSSEC resolution). The split-horizon authoritative approach replaces pi-hole's conditional-forwarding hack entirely: technitium IS authoritative for the LAN view of `okd.sudops.pl`, so `api.okd.sudops.pl` / `*.apps.okd.sudops.pl` queries resolve directly with no forwarding chain. Carry-over from pi-hole: `cluster.local` and `svc.cluster.local.okd.sudops.pl` as blocked/NXDOMAIN zones, plus a generous per-client rate limit (~10 k/min) so the cluster's nodes don't get REFUSED during DNS bursts. **Parallel option (not blocking)**: `tests/mc-nm-strip-okd-search.yaml` — MachineConfig that drops `okd.sudops.pl` from host resolv.conf, fixing the leak at the source; not applied because it requires an MCO master-pool reboot (~30-45 min). Worth doing eventually for cluster hygiene; not required for the technitium migration to succeed.
