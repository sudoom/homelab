...
- [ ] **GitHub OAuth IdP for cluster auth** — chart drafted at `components/cluster-config/oauth-idp/` (Helm + Argo-managed adoption of the singleton `OAuth/cluster` CR + GitHub IdP + optional htpasswd break-glass + cluster-admin RBAC). Not yet wired into `bootstrap/root-app/values.yaml`. Three remaining manual steps to finish the rollout — see `components/cluster-config/oauth-idp/README.md` for the runbook: (1) register the GitHub OAuth App at `https://github.com/organizations/sudoom/settings/applications/new` with callback `https://oauth-openshift.apps.okd.sudops.pl/oauth2callback/github`, (2) `kubeseal` the client secret to overwrite `templates/sealed-github-oauth-client-secret.yaml`, (3) fill in `clientID` + `clusterAdminUser` in `values.yaml` and enable the chart in root-app. `kubeadmin` stays available as the built-in fallback throughout.
