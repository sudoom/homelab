...
- [ ] **CephFS storage class** for ReadWriteMany workloads (currently RWO-only). **Drives arrived 2026-06-10** (ordered 2026-05-26): 10× HGST HUS726040**ALA610** 4TB SATA at 50 EUR/each (delivered model is the 512n ALA610, not the ALE610 the listing named — caught at SMART baseline; CMR either way). **Burn-in: 3 in-service drives PASSED** (K4KTAEDL→node4, K4KTD40L→node5, K7GEKUBR→node6 — long self-test "Completed without error", 0 reallocated/pending/offline/CRC, ~5 yr power-on; **badblocks dropped for the trio** — enterprise TLER drives behind size=3 + scrub + the SMART degradation alert `1dcc709`). Install **deferred** (node4's bay reused to burn in spares first). **Spare campaign underway**: batch 1 (K7GE897L/K7GE89HL/K4KZYB3B) PASSED + stored raw; batch 2 (K7GEX0MR/K7GEWZLR/K4KTDL9L) DDF-wiped-first + mid-long-test (ETAs ~15:38Z 2026-06-11); 6 of 7 spares processed/in-flight, 1 left to cycle. Hot-plug confirmed (live swaps, no reboot). **Used HGST pulls carry DDF firmware-RAID metadata** → wipe DDF (`mdadm --stop` + `wipefs -a`) first on every insertion, store shelf spares RAW (a broken DDF md array hangs cAdvisor → ~31 false *critical* etcd/CVO/TargetDown alerts; cluster was healthy throughout). Live run-log + per-drive SUMMARY in `data/hdd-burnin-2026-06-10/`. P5090 chassis has 1× 3.5" bay each → **3 active OSDs (1 per host) + 7 shelf spares**. ~4 TB usable post-size=3, shared with the RGW HDD pool. CephFS chart will ship one `CephFilesystem` with metadata pool on NVMe + two data pools (NVMe + HDD) per CLAUDE.md plan; bulk media stays on NFS/Synology (HDD tier sized for Immich + Jellyfin metadata + OADP, not video). Pre-install burn-in procedure + 4-phase install sequence drafted in `blog/blog-hdd-tier-rollout-draft.md`.
