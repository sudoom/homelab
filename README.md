...
- [ ] **Local pull-through image cache (Zot, RGW-backed)** — today every pod schedule pulls upstream from quay.io / docker.io / ghcr.io / registry.k8s.io. Any upstream registry hiccup mid-restart leaves pods Pending until it recovers. **Design**: single Zot Deployment (CNCF, OCI-native, ~100 MB RAM, single-binary) configured with S3 backend pointing at a `zot-cache` bucket on the existing `ceph-objectstore` RGW (same OBC+secret-translator pattern as Loki); cluster-wide `ImageDigestMirrorSet` rewrites pulls of common upstream registries through `zot.apps.okd.sudops.pl` (cached after first pull). Add Trivy scanning if/when there's appetite for CVE visibility on running workloads. **Why Zot over Harbor**: Harbor is ~10 pods (core, registry, redis, postgres, trivy, jobservice) which is overkill for a 3-node homelab; Zot delivers the resilience win at <1/10 the footprint. **Caveat**: the cluster's own bootstrap images (etcd, kube-apiserver, operators) can't be cached by a registry running INSIDE the cluster — chicken-and-egg. Cache helps workloads (Loki, ArgoCD, Grafana, media stack) but doesn't get us self-sufficient for cold cluster restart; that's a separate (bigger) project requiring an off-cluster mirror.
- [ ] **Power consumption reduction — lever 1 in measurement window, lever 2 queued** — 3 nodes + Mac mini + RPi run 24/7; idle waste is the easiest thing to cut once we can measure it. **Telemetry shipped 2026-05-21**: Shelly Plus Plug S Gen3 on node6 wall socket at `192.168.1.77`, scraped via `components/cluster-config/shelly-exporter/`. Pre-apply 24h baseline: 102.88 W avg, stddev 5.81 W. **Lever 1 sub-step A shipped 2026-05-23**: runtime-only Tuned profile (`components/cluster-config/power-tuning/`) — governor=powersave + EPB=15. 48h check (2026-05-25) confirmed +1.04 W vs baseline = noise. Hypothesis confirmed: on `intel_pstate=active`, the governor knob is decorative. **Lever 1 sub-step B shipped 2026-05-25**: `[bootloader]` section adds `intel_pstate=passive processor.max_cstate=9` via NTO-generated MachineConfig (master MCP, cluster-wide). 20-min serial reroll completed clean; `scaling_driver=intel_cpufreq` confirmed passive mode on all 3 nodes; idle cpu0 freq now at 800 MHz (= scaling_min). **+24h measurement clock**: 2026-05-26 ~15:35 UTC; `avg_over_time(shelly_power_watts[24h])` against the 102.88 W baseline. Decision tree: ≥5W cluster-wide gain → keep, optionally try `max_cstate=10`; <5W → CPU lever exhausted on this hardware, ship Lever 2 (Mac mini Jellyfin decommission, 10-30 W deterministic). Lever 3 (NVMe APST + PCIe ASPM + 10G EEE) remains queued. Don't bundle. Full chronology + post-cascade learnings in `blog/blog-power-consumption-draft.md`.
