...
### Queued — storage
- [ ] **Enable jumbo frames (MTU 9000) on the storage backnet** — `enp1s0f0np0` on each node carries Ceph OSD↔OSD replication + mon traffic (`network.provider: host` + `addressRanges: 192.168.10.0/24`). At MTU 1500, ~6× more packets/sec than necessary for the bulk replication workload; CPU + interrupt overhead on Ceph daemons. Jumbo would mostly help large sequential writes (RGW puts, scrub/repair, future bulk data); marginal for the current write-light homelab pattern but a real win when it matters and effectively free if the hardware supports it. **Pre-flight:** verify (a) Mellanox/Broadcom 10G NIC on each node supports MTU 9000 (almost certainly yes), (b) MikroTik switch L2MTU on VLAN 10 ports is high enough (default 1580 is too low — needs `interface ethernet set ... l2mtu=9214`), (c) any future device on this VLAN. **Apply via:** update `mtu: 9000` on the `enp1s0f0np0` interface entry in `components/cluster-config/nmstate-nncp/`; nmstate handler applies in-place (no reboot). Coordinate so ALL three nodes flip together — any single-side mismatch causes fragmentation/drops. Brief OSD↔OSD comms blip during interface re-up; Ceph heals in <30s. Independent of the 2026-05-21 OVN-K pod MTU change (different layer: pod overlay on frontnet vs physical interface on backnet).
...
- [ ] **Power consumption reduction** — 3 nodes + Mac mini + RPi run 24/7; idle waste is the easiest thing to cut once we can measure it. **Telemetry shipped 2026-05-21**: Shelly Plus Plug S Gen3 on node6 wall socket at `192.168.1.77`, scraped via `components/cluster-config/shelly-exporter/` (`prometheus-community/json_exporter` against `/rpc/Switch.GetStatus?id=0`). Metric series: `shelly_power_watts{instance="node6"}` + voltage/current/energy/temperature. Next step: capture a 24h idle baseline, then start ranking levers — (1) custom Tuned profile (governor=powersave + intel_pstate=passive + deeper C-states, ~15-25W/node), (2) Mac mini Jellyfin decommission (separate TODO, ~10-30W), (3) NVMe APST + PCIe ASPM + 10G EEE (small but free). Don't bundle changes — A/B each against baseline. Full design + ranked levers + stop-list in `blog/blog-power-consumption-draft.md`.
