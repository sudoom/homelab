...
- [ ] **CephFS storage class** for ReadWriteMany workloads (currently RWO-only). **Drives ordered 2026-05-26**: 10× HGST HUS726040ALE610 4TB SATA at 50 EUR/each. P5090 chassis has 1× 3.5" bay each → **3 active OSDs (1 per host) + 7 shelf spares**. ~4 TB usable post-size=3, shared with the RGW HDD pool. CephFS chart will ship one `CephFilesystem` with metadata pool on NVMe + two data pools (NVMe + HDD) per CLAUDE.md plan; bulk media stays on NFS/Synology (HDD tier sized for Immich + Jellyfin metadata + OADP, not video). Pre-install burn-in procedure + 4-phase install sequence drafted in `blog/blog-hdd-tier-rollout-draft.md`.
- [ ] **Flip RGW `dataPool.deviceClass` from `nvme` to `hdd`** — Phase 3 of the HDD-tier rollout (`blog/blog-hdd-tier-rollout-draft.md`). Single-line values change in `components/storage/ceph-object-store`; Rook updates the CRUSH rule, Ceph rebalances over the storage backnet, RGW endpoint + bucket names + client config unchanged. Metadata pool stays NVMe permanently. `pg_num_min` stays at 32 (3 HDD OSDs share the budget with CephFS-HDD).
...
- [ ] **Local pull-through image cache (Zot) — Phase A SHIPPED, Phase B (cluster-mirror wiring) queued.** Phase A landed `components/apps/zot/` — single StatefulSet, Zot v2.1.17, edge-terminated Route at `zot.apps.okd.sudops.pl`, on-demand sync from quay.io / docker.io / ghcr.io / registry.k8s.io. **Design tweak vs. the originally-tracked S3-backed shape**: shipped PVC-backed on `ceph-nvme-block` (50 Gi) instead of RGW-S3-backed. Rationale: simpler (one fewer secret-translator chain), lower cache-hit latency (kernel page cache + Zot dedupe inodes vs S3 GET per pull), and 50 GiB on NVMe is plenty for pull-through. Can be migrated to RGW-S3 later if PVC pressure or cross-pod-replica need ever shows up. **Phase B (queued, MCO-event scoped)**: ship cluster-wide `ImageDigestMirrorSet` + `ImageTagMirrorSet` to route quay.io/docker.io/ghcr.io/registry.k8s.io pulls through `zot.apps.okd.sudops.pl`, with `mirrorSourcePolicy: AllowContactingSource` for fallback. Triggers a 3-master MCO reroll — full cascade pre-flight required, schedule with 1h headroom. **Caveat unchanged**: the cluster's own bootstrap images (etcd, kube-apiserver, operators) can't be cached by a registry running INSIDE the cluster — chicken-and-egg. Cache helps workloads (Loki, ArgoCD, Grafana, media stack) but doesn't get us self-sufficient for cold cluster restart; that's a separate project requiring an off-cluster mirror. **Why Zot over Harbor (still applies)**: Harbor is ~10 pods (core, registry, redis, postgres, trivy, jobservice) which is overkill for a 3-node homelab; Zot delivers the resilience win at <1/10 the footprint.
