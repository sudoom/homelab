...
- [ ] **PrometheusRule: `rook-ceph-rgw-*` Service endpoint availability** — surfaced by the 2026-05-13 RGW silent 21h outage. Alert on `kube_endpoint_address_available{namespace="rook-ceph", endpoint=~"rook-ceph-rgw-.*"} == 0` for >2m → "RGW Service has no healthy endpoints". Would have caught the EADDRINUSE crashloop in minutes rather than during a manual sweep. Add to `components/cluster-config/monitoring-config/` alongside the existing PrometheusRules. Generalize to all `rook-ceph-*` Service endpoints if cheap.
