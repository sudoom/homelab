...
- [ ] **`logging-loki-ingester-2` WAL %full draining asynchronously after pod restart** — surfaced by 2026-05-13 end-of-session sweep: `wal-logging-loki-ingester-2` PVC at 99.83% used (~10Gi quota), zero delta over 30 min, flush rate stuck at 0.033/sec. Likely accumulated during the 21h RGW outage. Mitigation applied 2026-05-13 14:55 CEST: `oc -n openshift-logging delete pod logging-loki-ingester-2`. New pod came back Ready at 15:01 CEST (4m 27s WAL replay). Flush rate restored to ~2.7/sec peaks (80× pre-restart). **WAL %full metric still at 99.83% immediately post-restart** — Loki WAL is segment-based; truncation lags actual flushes (checkpoint advances on a schedule, not per-flush). Expected to drain over the next 1-2 hours. Two `KubePersistentVolumeFillingUp` alerts (critical + warning) should auto-resolve when the metric drops. **If still at 99.83% after ~4h:** dig into ingester WAL checkpoint config — there may be a chunk-per-checkpoint or time-per-checkpoint limit that's not advancing.
