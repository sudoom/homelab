...
- [ ] **Storage stack re-bootstrap on `provider: host` + `addressRanges` — bootstrap shipped 2026-05-15, consumer re-enable pending** — fresh `CephCluster` came up `HEALTH_OK` in ~3.5 min on `provider: host` + `network.addressRanges.public/cluster: ["192.168.10.0/24"]`; OSDs bound to `192.168.10.{2,3,4}:6800` (10G backnet `enp1s0f0np0`) from first boot, confirmed via `ceph osd dump`. Two follow-up issues bit before the libaio bench could run: (1) **mons bound to frontnet `192.168.1.{7,8,9}:6789`** despite `addressRanges` — Rook tracks pod IP (kubelet-registered nodeIP, which is the 1G frontnet) for mon-endpoint advertisement rather than honoring the Ceph config-DB `public_network`. Mon traffic is small (clustermap + heartbeat) so shouldn't dominate, but it's a design deviation worth chasing. (2) **`client.csi-rbd-provisioner.1` was generated by Rook with missing `osd profile rbd` cap** — fixed by toolbox `ceph auth caps` (see new bug draft `bugs/upstream-rook-csi-rbd-provisioner-missing-osd-cap.md`). After both worked around, PVC binds. **Still queued**: libaio bench → 10G-host benchmark commit; Phase 7 consumer re-enable (`monitoring-config`, `logging-stack`, `grafana-config`, `gatus`, `media`); media-data-pvc NFS rebind. Multus parked indefinitely; if it ever comes back, `kubectl-rook-ceph multus validation run` is mandatory pre-flight per CLAUDE.md. Full chronology in `blog/blog-multus-ceph-migration-draft.md`.
