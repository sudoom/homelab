...
components/
  ...
  apps/
    cnpg-clusters/         # Per-namespace CNPG Cluster CRs (currently: media-postgres,
                           #   3-instance PG 18, 8 DBs for the servarr stack)
    media/                 # Media stack: 4 servarrs (sonarr/radarr/prowlarr/bazarr) + jellyfin
                           #   + transmission; first 3 on Postgres via media-postgres
...
- [ ] **Migrate apps from old cluster (media stack + keepers)** — port over the workloads still running on the previous cluster. Media stack now lives in tree (`components/apps/media/`); 3 of the 4 servarrs (Sonarr/Radarr/Prowlarr) on shared CNPG Postgres as of 2026-05-15. Remaining migration work is **config-only** (no DB data to bring across): re-apply each app's config via its web UI on the new cluster, re-seal each keeper Secret on the new cluster's sealed-secrets controller (per-controller pubkey means old blobs don't transplant). Pattern matches the in-repo Cloudflare token + GitHub OAuth client secret.
- [ ] **Bazarr on Postgres** — Bazarr is Python/Flask, takes different env var names than the .NET servarrs (`POSTGRES_ENABLED=True`, `POSTGRES_DATABASE`, `POSTGRES_USERNAME`, etc., not `Sonarr__Postgres__*`). Today's `components/apps/media/templates/apps.yaml` only wires the .NET shape. Needs a template branch (or per-app env override) before flipping Bazarr's `postgres.enabled`. Low priority — Bazarr's SQLite is fine for the workload.
- [ ] **CNPG: barmanObjectStore for `media-postgres`** — wire WAL archiving + base backups to the existing `ceph-objectstore` RGW. Pattern: `CephObjectStoreUser` (separate from Loki), OBC + secret-translator (same shape as Loki's), `spec.backup.barmanObjectStore` on the Cluster CR pointing at `rook-ceph-rgw-ceph-objectstore.rook-ceph.svc:80`. Enables PITR for all servarr DBs at one stroke. Queued because the data is fresh + low-value today; bake before declaring DR ready.
