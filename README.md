...
### In flight
- [ ] **Drop `okd.sudops.pl` from the node-side / pod DNS search list** — promoted from `Queued — platform plumbing` (2026-05-08): now confirmed as the root cause of the Loki read-path 504, not just a pi-hole noise issue. Pod resolv.conf inherits `search okd.sudops.pl` and `options ndots:5`. The Loki querier resolves `logging-loki-index-gateway-grpc.openshift-logging.svc.cluster.local` (4 dots, < ndots:5), which makes glibc try the search list first — including `<FQDN>.okd.sudops.pl` against pi-hole, which stalls ~5 s. gRPC client deadline is 5 s → `DeadlineExceeded` → `pool.go:250` eviction loop → 504s on `Loki (infrastructure)` / `Loki (audit)` queries. Direct pod-IP TCP connects from the querier complete in <1 ms; FQDN connects take 6003 ms. Fix is a `MachineConfig` adjusting `NetworkManager` `dns-search`/`ignore-auto-dns` so the suffix doesn't land in node resolv.conf (and from there, in pod resolv.conf). Also worth reducing `ndots` on cluster pods if cheap, but the search-list removal is the single load-bearing change.
...
- [ ] **Logging stack read-path 504 (querier ↔ index-gateway gRPC pool)** — write path is verified end-to-end (Vector → gateway → ingester → RGW; bucket grew steady through diagnosis). Grafana queries against `Loki (infrastructure)` and `Loki (audit)` return 504 because the querier evicts the index-gateway from its connection pool every ~10s with `pool.go:250 reason="rpc error: code = DeadlineExceeded"`. **Root cause confirmed 2026-05-08**: the `okd.sudops.pl` search-list + `ndots:5` makes every gRPC dial against the index-gateway FQDN take ~6 s for DNS resolution, exceeding gRPC's 5 s deadline. *Both earlier hypotheses were wrong*: not the PNY OSDs (still failed after the full PM9A1 swap on 2026-05-07), and not the Loki Operator `1x.pico` bug. Resolution depends on the search-list-cleanup item now in `In flight`. Full diagnosis chronology in `blog/blog-loki-logging-draft.md`; correction note appended to `blog/blog-rook-ceph-draft.md`.
...
- [ ] **Multus migration for Ceph clients** — drafted in `blog/blog-multus-ceph-migration-draft.md`. Macvlan NAD over `enp1s0f0np0`, flip `network.provider: multus`, rolling daemon restart. Pre-flight already passes (Multus + whereabouts present). **Rationale flipped post-PM9A1 swap (2026-05-07)**: the 1 GbE frontnet is now genuinely close to saturating on 1M QD8 seqwrite (router peaked ~600 Mb/s during dd), so moving client→primary traffic onto the 10 GbE backnet is expected to lift 1M seqwrite ~173 MB/s → ~700 MB/s and random read 40 k → ~100 k IOPS. No effect on QD1 fsync or QD32 randwrite ceilings (per-op replication latency dominates those, network is < 5 % of the floor). Validate by re-running `tests/ceph-storage-test-libaio.yaml` post-migration and comparing the seqread/seqwrite numbers in the bottleneck-sweep table in `blog/blog-rook-ceph-draft.md`.
...
- [ ] **Repo public-readiness pass** — drafts at `blog/blog-*-draft.md` and `nmstate-imagestream-bug.md` currently committed; review for anything that shouldn't be public before next push to GitHub.